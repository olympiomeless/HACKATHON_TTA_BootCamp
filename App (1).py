...
@app.route("/api/charts/recommendations-by-segment.png")
def chart_recommendations_by_segment():
    """
    Catégories les plus recommandées par groupe d'âge et par genre
    (issu du notebook section 4.1).
    Paramètre optionnel : ?segment=age (défaut) | gender
    """
    segment = request.args.get("segment", "age")  # 'age' ou 'gender'

    # Construire les groupes d'âge
    profiles = rec_engine.user_profiles.reset_index().copy()
    profiles['AgeGroup'] = pd.cut(
        profiles['Age'],
        bins=[18, 30, 45, 65],
    ...
    # Générer une recommandation top-1 par utilisateur
    recommendations_data = []
    for _, row in profiles.iterrows():
        user_id = row['UserID']
        suggestions = rec_engine.get_personalized_suggestions(user_id, num_suggestions=1)
        if not suggestions:
            continue
        raw = suggestions[0]
        cat = (raw
               .replace('Livre de ', '')
               .replace('Livre populaire de ', '')
               .split(':')[0].strip())
        if 'similaire' in cat:
            try:
                cat = cat.split('en ')[1].strip()
            except IndexError:
                cat = 'Autre'
        if 'spéciale' in cat:
            cat = 'Biographie/Histoire'
    ...
    if segment == "gender":
        grouped = rec_df.groupby('Gender')['RecommendedCategory'].value_counts().unstack(fill_value=0)
        title   = "Catégories les plus recommandées par genre"
    else:
        grouped = rec_df.groupby('AgeGroup')['RecommendedCategory'].value_counts().unstack(fill_value=0)
        title   = "Catégories les plus recommandées par groupe d'âge"
    ...
    for i, (group_val, data) in enumerate(grouped.iterrows()):
        top = data.nlargest(5)
        sns.barplot(
        ...
        axes[i].set_title(str(group_val), fontsize=11)
        axes[i].set_xlabel('Catégorie')
        axes[i].set_xticks(range(len(top.index)))
...
if __name__ == "__main__":
    ...
    print("    GET  /api/charts/recommendations-by-segment.png?segment=age")
    print("    GET  /api/charts/recommendations-by-segment.png?segment=gender")
    print()
